package io.github.the_infinite.core.env;
...
public class AppEnvironment {
    ...
    public static void withEnvFile(Vertx vertx, @Nullable String path) throws IllegalArgumentException {
        ...
        var serverName = instance.get("SERVICE_NAME", null);
        var versionCode = instance.get("SERVICE_VERSION", null);
        var buildNumberStr = instance.get("BUILD_NUMBER", null);
        var envKind = instance.get("ENVIRONMENT_KIND", "PRODUCTION");
        var processRole = instance.get("PROCESS_ROLE", "WORKER");
        var setupMode = instance.get("SETUP_MODE", "PRODUCTION");
        var serverPort = instance.get("SERVER_PORT", null);
        var urlStr = instance.get("SERVICE_URL", null);

        //? Second, for the database and service config settings.
        var pgUrl = instance.get("POSTGRESQL_URL", null);
        var redisUrl = instance.get("REDIS_URL", null);
        var rabbitMqUrl = instance.get("RABBITMQ_URL", null);
        var mongoDbUrl = instance.get("MONGODB_URL", null);
