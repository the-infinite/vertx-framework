  ...
  public void registerRoutes() {
   ...
   private void handleDocumentationRequest(io.vertx.ext.web.RoutingContext context) {
     ...
     if (mode == DocumentationRegistrant.DocumentationMode.EXTERNAL) {
       htmlContent = renderExternalHtml(registrant, context.request().path());
     } else {
...
${jsonIndent(depth)}${jsonPunctuation('}')}`;
   ...
   private String renderExternalHtml(DocumentationRegistrant registrant, String documentationPath) {
     StringBuilder html = new StringBuilder();
     boolean isProduction = isProductionEnvironment();
     List<HttpClient> availableHttpClients = getAvailableHttpClients(registrant);
     String defaultClientId = normalizeClientName(new ClassicHttpClient().getName());
     ...
     html.append("const DOCUMENTATION_PATH = '").append(escapeForJsSingleQuotedString(documentationPath)).append("';");
     ...
     html.append("function trimTrailingSlash(path) { if (!path || path === '/') { return ''; } return path.endsWith('/') ? path.slice(0, -1) : path; }");
     html.append("function resolveServiceBasePath() { const path = window.location.pathname || DOCUMENTATION_PATH || '/'; return trimTrailingSlash(path); }");
     html.append("function joinUrlPath(basePath, routePath) { if (!routePath) { return basePath || '/'; } const isAbsoluteHttpUrl = routePath.startsWith('http://') || routePath.startsWith('https://'); if (isAbsoluteHttpUrl) { return routePath; } const normalizedBase = trimTrailingSlash(basePath || ''); const normalizedRoute = routePath.startsWith('/') ? routePath : '/' + routePath; if (normalizedBase && (normalizedRoute === normalizedBase || normalizedRoute.startsWith(normalizedBase + '/'))) { return normalizedRoute; } return normalizedBase + normalizedRoute; }");
